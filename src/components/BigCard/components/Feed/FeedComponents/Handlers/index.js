...
export const sortPostsByVote = posts => {
    ...
    posts.forEach(post => {
        allPosts.push({
            ...post,
            votes: (post.upvotes ? post.upvotes.length : 0) - (post.downvotes ? post.downvotes.length : 0),
        })
...
export const upVoteIncrease = async (postID, userID) => {
    try {
        const res = await API.graphql(graphqlOperation(votePost, { postID, userID, type: "UPVOTE" }))
...
export const downVoteIncrease = async (postID, userID) => {
    try {
        const res = await API.graphql(graphqlOperation(votePost, { postID, userID, type: "DOWNVOTE" }))
        return res.data.votePost
    }
    catch (err) {
        return {
